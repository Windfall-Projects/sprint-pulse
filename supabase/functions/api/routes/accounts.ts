...
app.post('/', zValidator('json', CreateAccountSchema), async (c) => {
    ...
    // 1. Create the account without select() to avoid RLS error before membership exists
    ...
    // 3. Fetch and return the fully created and accessible account
    ...
    if (fetchError || !account) {
        return c.json({ error: 'Account created but failed to retrieve details.' }, 500);
    }
