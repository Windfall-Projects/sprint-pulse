...
describe('7 · Surveys', () => {
    ...
    describe('SurveySchema', () => {
        it('accepts valid survey', () => {
            const s = { id: UUID, account_id: UUID, team_id: UUID, title: 'Retro', description: null, is_system_template: false, trigger_event: null, created_at: ISO_TS };
            expect(SurveySchema.safeParse(s).success).toBe(true);
        });
        it('accepts null team_id (system template)', () => {
            const s = { id: UUID, account_id: UUID, team_id: null, title: 'System', description: 'System Template', is_system_template: true, trigger_event: null, created_at: ISO_TS };
            expect(SurveySchema.safeParse(s).success).toBe(true);
        });
    });
