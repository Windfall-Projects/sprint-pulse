...
app.post('/webhook', async (c) => {
    ...
    if (event === 'issues') {
        ...
        if (!parsed.success) {
            return c.json({ message: 'Invalid payload', errors: parsed.error.format() }, 400);
        }
        ...
        const account_id = Array.isArray(mapping.integrations)
            ? mapping.integrations[0]?.account_id
            : mapping.integrations?.account_id;

        if (!account_id) {
            return c.json({ message: 'Mapping has no associated account_id' }, 400);
        }
